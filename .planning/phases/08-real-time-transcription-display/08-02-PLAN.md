...
must_haves:
  truths:
    ...
    - "Clicking the display dismisses/minimizes it"
  artifacts:
...
**Event listeners (for now, use Tauri events as mock interface - Plan 03 will wire Channels):**
...
**Click to dismiss:**
Add onClick handler on the outer container that emits a hide event or calls setIsVisible(false). Use `getCurrentWindow().hide()` from `@tauri-apps/api/window`.
